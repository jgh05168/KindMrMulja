...
delivery.post("/delivery-address/add", async (req, res) => {
  ...
  try {
    ...
    const result2 = await global.pool.query(query3, user_id);
    ...
    if (result[0].affectedRows > 0) {
      if (result2[0].length > 0) {
        ...
        if (is_default === "1") {
          await global.pool.query(query2, result2[0][0].address_id);
        }
      }
...
delivery.delete("/delivery-address/:address_id", async (req, res) => {
  ...
  try {
    ...
    const result = await global.pool.query(query, [address_id]);
