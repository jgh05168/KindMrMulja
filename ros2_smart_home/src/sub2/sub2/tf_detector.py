...
def main(args=None):
    ...
    back_folder = 'Desktop\\PJ\\S10P22C109\\ros2_smart_home\\src\\sub2'

    CWD_PATH = os.path.join(pkg_path, back_folder, 'sub2')
    ...
    while rclpy.ok():
        ...
        if len(boxes_detect) != 0:
            ...
            for i in range(bbox.shape[0]):
                ...
                xyv = xyz[np.where((xyz[:, 0] >= x) & (xyz[:, 0] <= x + w) & (xyz[:, 1] >= y) & (xyz[:, 1] <= y + h))]

                    # 대표값이 존재하면
                if xyv.shape[0] > 0:
                    ostate = np.mean(xyv, axis=0)
