...
class odom(Node):
    ...
    def imu_callback(self, msg):
        # 로직 3. IMU에서 받은 quaternion을 euler angle로 변환해서 사용
        if not self.is_imu:
            self.is_imu = True
    ...
    def listener_callback(self, msg):
        ...
        print(self.initial_x_is,self.initial_y_is)
        ...
        if self.is_imu ==True:
            if self.is_status == False :
                ...
                self.x += linear_x * math.cos(self.theta) * self.period
                self.y += linear_x * math.sin(self.theta) * self.period
                ...
                # odometry 메세지도 이동, 회전, 제어 값을 채워준다.
